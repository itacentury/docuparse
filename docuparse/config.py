...
2. Die Kategorie des Einkaufs. Zum Beispiel 'Lebensmittel',
'Restaurant' oder 'Elektronik'.
3. Datum ohne Uhrzeit im ISO-8601 Format.
4. Gesamtpreis.
5. Alle Artikel inklusive Preis, Artikel in korrekter deutschen
Groß- und Kleinschreibung.

Wenn der gleiche Artikel mehrfach gekauft wurde, dann schreibe
als Preis für den Artikel den zusammengerechneten Preis
und füge auch die Anzahl vor dem Artikelnamen hinzu
(z.B. '4x Semmel'), außer bei Pfand.
Wenn ein Artikel Pfand hat, addiere den dazugehörigen Pfand
zum Preis des Artikels.
Wenn ich bei einem Einkauf Pfand zurückgebe, behandle dies als
Negativzahl und fasse jeden zurückgegebenen Pfand unter einem
einzigen Eintrag 'Pfand' zusammen.
Schreibe das Gewicht bei zum Beispiel Gemüse oder Obst, hinten
an den Namen des dazugehörigen Gemüse oder Obstes.

Gebe mir die Daten im JSON-Format zurück, mit folgenden Namen
und Datentypen:
'store' (str), 'category' (str), 'date' (str),
'items' (list[dict[str, str | float]]), 'total' (float).
